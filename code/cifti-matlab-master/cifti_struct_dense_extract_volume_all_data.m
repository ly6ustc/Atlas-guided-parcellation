function [outdata, outsform1, outroi] = cifti_struct_dense_extract_volume_all_data(cifti, cropped, dimension)
    %function [outdata, outsform1, outroi] = cifti_struct_dense_extract_volume_all_data(cifti, cropped, dimension)
    ...
    if otherlength == 1 %don't loop if we don't need to
        ...
        tempframe = zeros(volinfo.voldims, 'single'); %%%%revised by ly
