function labels=region_growing_ly_1_hom(seeds, similarity, neibors, threshold)
...
%         tic;
        ...
        if t< threshold && (2*(Hom(indexf) + Vertex(indexf,1)*Num_vertex(indexf))/(Num_vertex(indexf)*(Num_vertex(indexf)+1)))<threshold
            clustertemp(indexf)=[];
            S_all(indexf)=[];
            Vertex(indexf,:)=[];
            cluster=clustertemp;
            ...
            Hom(indexf)= Hom(indexf)+Vertex(indexf,1)*Num_vertex(indexf);
